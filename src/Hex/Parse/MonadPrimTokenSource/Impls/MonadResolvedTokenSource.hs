...
module Hex.Parse.MonadPrimTokenSource.Impls.MonadResolvedTokenSource where
...
instance H.Par.TokSrc.MonadResolvedTokenSource m => Alternative (ParseT m) where
...
parseTry :: forall m a. (Monad m, H.Par.TokSrc.MonadResolvedTokenSource m) => ParseT m a -> ParseT m a
...
instance (H.Par.TokSrc.MonadResolvedTokenSource m, MonadError e m, AsType ExpansionError e) => MonadPrimTokenSource (ParseT m) where
...
fetchPrimitiveToken :: (H.Par.TokSrc.MonadResolvedTokenSource m, MonadError e m, AsType ExpansionError e) => m PrimitiveToken
...
instance H.Par.TokSrc.MonadResolvedTokenSource m => MonadPlus (ParseT m)
