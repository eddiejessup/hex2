...
getLetterChars :: H.St.MonadHexState m => BS.ByteString -> m (Seq Code.CharCode, BS.ByteString)
getLetterChars = go Seq.Empty
  where
    go acc cs = runExceptT @H.Cat.CatFailure (H.Cat.extractCharCat cs) >>= \case
...
dropTilEndOfLine :: H.St.MonadHexState m => BS.ByteString -> m BS.ByteString
...
extractToken
  ...
  => LexState
  -> BS.ByteString
  -> m (LexToken, LexState, BS.ByteString)
extractToken = go
  where
    go :: LexState -> BS.ByteString -> m (LexToken, LexState, BS.ByteString)
    go _state cs = do
          ...
          runExceptT @H.Cat.CatFailure (H.Cat.extractCharCat rest) >>= \case
            ...
            Right (H.Cat.RawCharCat csChar1 ctrlSeqCat1, restPostEscape) -> do
              (ctrlSeqChars, restPostCtrlSeq) <- case ctrlSeqCat1 of
                    ...
                    _ ->
                      pure (Seq.singleton csChar1, restPostEscape)
...
charsToLexTokens :: forall m. H.St.MonadHexState m => BS.ByteString -> ExceptT LexError m [LexToken]
charsToLexTokens = go LineBegin
  where
    go lexState xs = runExceptT (extractToken lexState xs) >>= \case
...
-- usableCharsToLexTokens :: BS.ByteString -> Either LexError [LexToken]
